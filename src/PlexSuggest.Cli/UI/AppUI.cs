...
public static class AppUI
{
    ...
    static void ShowDetail(ScoredItem scored)
    {
        ...
        T.Write(new VStack(
            ...
            new TextBlock($"  Genres: {string.Join(", ", m.Genre.Select(g => g.Name))}"),
            new TextBlock($"  Director: {string.Join(", ", m.Director.Select(d => d.Name))}"),
            new TextBlock($"  Cast: {string.Join(", ", m.Role.Select(a => a.Name).Take(5))}"),
            scoreTb,
            reasonTb,
            new TextBlock(""),
            summaryTb,
